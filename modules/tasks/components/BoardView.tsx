...
const PRIORITY_COLORS = {
  LOW: "bg-blue-50 text-blue-600 border-blue-150",
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 overflow-x-auto pb-4">
      {STATUSES.map((colStatus) => {
        ...
        return (
          <div
            key={colStatus}
            className="bg-slate-50 border border-slate-100 rounded-xl p-4 min-w-[250px] flex flex-col h-full"
          >
            {/* Column header */}
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[500px]">
              {filteredTasks.length === 0 ? (
                <div className="border border-dashed border-slate-200 rounded-lg p-6 text-center text-[10px] text-slate-400">
                  Empty column
                </div>
              ) : (
                filteredTasks.map((task) => (
                  <div
                    ...
                    className="bg-white border border-slate-100 rounded-lg p-3.5 shadow-[0_1px_4px_rgba(0,0,0,0.015)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.04)] hover:border-slate-200 cursor-pointer transition-all duration-200 space-y-3"
                  >
                    <div className="flex justify-between items-start gap-1">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 bg-slate-55 border border-slate-100 px-1.5 py-0.5 rounded max-w-[80px] truncate">
                        {task.project.name}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-slate-50">
                      ...
                      <div className="flex items-center gap-1">
                        {task.assignee ? (
                          ...
                          <div
                            title="Unassigned"
                            className="h-5.5 w-5.5 rounded-full bg-slate-100 border border-slate-200 text-slate-400 flex items-center justify-center"
                          >
